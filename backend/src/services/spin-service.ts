...
import { createPcgRng } from './pcg-rng.js';
import type { PcgRng } from './pcg-rng.js';
...
function generateConstrainedGrid(wildCount: number, rng: PcgRng): SymbolId[][] {
    const grid: SymbolId[][] = [];
...
export function generateBuyBonusSpin(wildCount: number, totalBet: number): SpinServiceResult {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        const grid = generateConstrainedGrid(wildCount, rng);
